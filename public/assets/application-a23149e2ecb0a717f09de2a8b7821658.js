...
(function() {
  ...
  user = {
    ...
    getQuestions: function() {
      return $('#check-question').on("click", function() {
        var username;
        $(".question").remove();
        ...
        $.ajax({
          ...
          $(".alert").remove();
          if (data.not_errors) {
            return $.each(data.questions.items, function(i, e) {
              var quest;
              quest = _.template($.trim($("#question").html()), {
                title: e.title,
                tags: e.tags,
                link: e.link
              });
              return $("#result").prepend(quest);
            });
          } else {
            return $("#result").append("<div class='alert alert-danger'>This user not exist on StackOverflow</div>");
          }
        });
        return setTimeout("$('.alert').hide();", 4000);
      });
    }
  };
